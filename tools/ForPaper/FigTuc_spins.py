...
name = path+'ForPaper/TUC_spin/*.txt'
...
def process(datafile,Tintegration,color):
   ...
   # ax1.loglog(f,np.sqrt(hsig),C =plotC, label = iota)
...
index = 0
for f in datafiles:
    SNR,index = process(f,Tint,index)
...
#plt.savefig(savepath+'MW_SpectraCompareInclination', dpi=300)
